...
function installToMediaWikiRoot {
	...
	then
		...
		composer require 'phpunit/phpunit=7.5.*' --update-with-dependencies
	fi

	if [ "$WNBY" != "" ]
	then
		composer require 'mediawiki/whats-nearby='$WNBY --update-with-dependencies
	else
		composer init --stability dev
		composer require "mediawiki/whats-nearby:dev-master" --dev --update-with-dependencies
